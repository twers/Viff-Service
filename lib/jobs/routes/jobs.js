...
module.exports = function (Jobs, Job) {
  ...
  function jobCreateValidator (formParams){
  ...
  routes.createFormValidation = function (req, res, next) {
    ...
    var validationErrors = jobCreateValidator(formParams);
  ...
  routes.updateFormValidation = function(req, res, next) {
    ...
    if (req.files.configFile.size !== 0 && !req.files.configFile.originalFilename.match(/\.js$/)) {
      fileError = true;
    }
