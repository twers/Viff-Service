module.exports = function(Jobs) {
  ...
  routes.createFormValidation = function(req, res, next) {
    ...
    if (!uploadFile.configFile || !uploadFile.configFile.path) {
      fileError = { param: 'configFile', msg: 'null file', value: '' };
    } else if (!uploadFile.configFile.originalFilename.match(/\.js$/)){
    ...
    if (fileError || req.validationErrors() && req.validationErrors().length) {
      return res.format({
        html: function() {
          res.render('new-job', {
            title: 'Create A new Viff Job',
            errors: (req.validationErrors()).push(fileError)
          });
        },
        json: function() {
