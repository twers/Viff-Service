...
module.exports = {
  ...
  log: function(req, res) {
    ...
    Jobs.id(jobId, function(err, job) {
      ...
      var stream = runner.loadLog(job.get('name'), buildId, end);
      stream.pipe(res);
    });
  },
  ...
  run: function (req, res) {
    ...
    Jobs.id(jobId, function (err, JobObj) {
      ...
      var viff = runner.run(jobName, buildNumber, req.build.get('config'));
  ...
  terminate: function(req, res) {
    ...
    Jobs.id(jobId, function(err, job) {
      ...
      runner.terminate(job.get('name'), buildId, function(err) {
        if (err) {
